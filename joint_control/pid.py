...
class PIDController(object):
    ...
    def __init__(self, dt, size):
        ...
        delay = 2  # Simulation delay steps
        # delay = 0
        # self.Kp = 0
        # self.Ki = 0
        # self.Kd = 0
    ...
    def control(self, target, sensor):
        ...
        # Current error
        e0 = (target - sensor) * (1 + DAMPING) + BIAS

        # PID calculations
        proportional = self.Kp * e0
        integral = self.Ki * (e0 + self.e1) * self.dt / 2
        if self.dt == 0:  # to avoid division by 0 error, set derivative to infinity
            derivative = inf
            # derivative = 1000000
        else:
            derivative = self.Kd * (e0 - self.e1) / self.dt

        # Update error
        self.e2 = self.e1 * (1 + DAMPING) + BIAS
        self.e1 = e0 * (1 + DAMPING) + BIAS
